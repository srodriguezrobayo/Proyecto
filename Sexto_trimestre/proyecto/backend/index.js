...
app.post('/cliente/registro', (req, res) => {
    let { 
        id_Cliente,Nombre_cliente,Correoelectronico_cliente,Password_cliente,Telefono_cliente,Genero_idGenero,Ciudad_id_Ciudad,Foto_cliente
    } = req.body

    const sql = 'INSERT INTO cliente (id_Cliente, Nombre_cliente, Correoelectronico_cliente, Password_cliente, Telefono_cliente, Genero_idGenero, Ciudad_id_Ciudad, Foto_cliente) VALUES (?,?,?,md5(?),?,?,?,"https://i.imgur.com/3J4lKnO.png")';
    conexion.query(sql, [id_Cliente, Nombre_cliente, Correoelectronico_cliente, Password_cliente, Telefono_cliente,Genero_idGenero,Ciudad_id_Ciudad,Foto_cliente], (error, resultado) => {
        if (error) {
...
app.post('/empresa/registro', (req, res) => {
    ...
    const sql = 'INSERT INTO empresa (Nit_Empresa, Nombre_empresa, Correoelectronico_empresa, Password_empresa, Desc_empresa, Telefono_empresa, Ciudad_id_Ciudad, Foto_empresa) VALUES (?,?,?,md5(?),?,?,?,"https://i.imgur.com/3J4lKnO.png")';
...
app.post('/reserva', (req, res) => {
    try {
        ...
        if (Servicio_idServicio === '6') {
            Empleados_id_empleado = '74';
        } else if (Servicio_idServicio === '7') {
            Empleados_id_empleado = '55';
        } else if (Servicio_idServicio === '8') {
            Empleados_id_empleado = '92';
        } else if (Servicio_idServicio === '9') {
            Empleados_id_empleado = '100';
        } else if (Servicio_idServicio === '5' || Servicio_idServicio === '10') {
            Empleados_id_empleado = '79';
        }
        ...
        conexion.query(query, [Servicio_idServicio, Fecha_reservacion, Hora_reservacion, Lugar_reservacion_idLugar_reservacion, Empresa_Nit_Empresa, Cliente_id_Cliente, Valor, Empleados_id_empleado], (error) => {
            if (error) {
                console.error(error.message);
                res.status(500).json({ error: 'Error en la reserva' });
            } else {
...
app.post('/subir_img', upload.single('file'),(req,res,next)=> {
    const file = req.file;
...
app.put('/cliente/update/:id', (req, res) => {
    const { id } = req.params;
    const { Nombre_cliente, Correoelectronico_cliente, Password_cliente, Telefono_cliente, Genero_idGenero, Ciudad_id_Ciudad, Foto_cliente } = req.body;
  
    const query = `
      UPDATE cliente 
      SET 
        Nombre_cliente = '${Nombre_cliente}', 
        Correoelectronico_cliente = '${Correoelectronico_cliente}', 
...
app.put('/empresa/update/:id', (req, res) => {
    ...
    const query = `UPDATE empresa SET Nombre_empresa ='${Nombre_empresa}', Correoelectronico_empresa = '${Correoelectronico_empresa}', Password_empresa = '${Password_empresa}', Desc_empresa = '${Desc_empresa}', Telefono_empresa = '${Telefono_empresa}', Ciudad_id_Ciudad = '${Ciudad_id_Ciudad}' WHERE Nit_Empresa = '${id}';`;
    conexion.query(query, (error) => {
        if (error) {
            console.error(error.message);
            res.status(500).json({ error: 'Hubo un error al actualizar la empresa' });
